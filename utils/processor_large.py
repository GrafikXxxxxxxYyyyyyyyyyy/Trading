...
class TradingProcessor:
    ...
    def _compute_price_changes(self, close):
        ...
        abs_close_change = torch.cat([torch.zeros_like(close[:, :1]), close[:, 1:] - close[:, :-1]], dim=1) # 13
        features.append(abs_close_change)
        
        # Относительное изменение цены закрытия (%)
        rel_close_change = self._safe_division(abs_close_change, torch.roll(close, shifts=1, dims=1)) # 14
        features.append(rel_close_change)

        # Логарифмическое изменение
        log_close_change = torch.log(torch.roll(close, shifts=-1, dims=1) / (close + self.eps) + self.eps) # 15
    ...
    def _compute_volatility_indicators(self, high, low, close, window=14):
        ...
        tr = torch.maximum(torch.maximum(high_low, high_close_prev), low_close_prev) # 16
        ...
        volatility = compute_std(close.transpose(-1, -2), window) # 18
        ...
        bb_mean, bb_upper, bb_lower = compute_bollinger_bands(close.transpose(-1, -2), window) # 19, 20, 21
        bb_width = bb_upper - bb_lower # 22
        bb_position = self._safe_division(close - bb_lower, bb_width) # 23
    ...
    def _compute_moving_averages(self, close, volumes, open_p, high, low):
        ...
        sma_5 = compute_sma(close_t, 5) # 27
        sma_15 = compute_sma(close_t, 15) # 28
        sma_30 = compute_sma(close_t, 30) # 29
        features.extend([sma_5, sma_15, sma_30])

        # EWMA (приблизительные)
        ewma_5 = compute_ewma_approx(close, 5) # 30
        ewma_15 = compute_ewma_approx(close, 15) # 31
        ewma_30 = compute_ewma_approx(close, 30) # 32
        ...
        vwap = self._safe_division(vwap_numerator, vwap_denominator).transpose(-1, -2) # 33
        features.append(vwap)

        # MACD
        ema_12 = compute_ewma_approx(close, 12)
        ema_26 = compute_ewma_approx(close, 26)
        macd_line = ema_12 - ema_26 # 34
        signal_line = compute_ewma_approx(macd_line, 9) # 35
        macd_histogram = macd_line - signal_line # 36
        ...
        hh_20, ll_20 = compute_price_channels(high_t, low_t, 20) # 37, 38
    ...
    def _compute_momentum_indicators(self, close, window=14):
        ...
        # Rate of Change (ROC)
        roc = self._safe_division(close - torch.roll(close, shifts=window, dims=1), torch.roll(close, shifts=window, dims=1)) # 39
        ...
        williams_r = -100 * self._safe_division(williams_r_numerator, williams_r_denominator) # 40
        features.append(williams_r)

        # Momentum
        momentum = close - torch.roll(close, shifts=window, dims=1) # 41
        momentum[:, :window] = 0.0
        features.append(momentum)

        # Price Rate of Change (PROC) - нормализованная версия Momentum
        proc = self._safe_division(momentum, torch.roll(close, shifts=window, dims=1)) # 42
    ...
    def _compute_oscillators(self, close, high, low, volumes, window=14):
        ...
        avg_gain = rma(gain, window) # 43
        avg_loss = rma(loss, window) # 44
        ...
        stoch_k = 100 * self._safe_division(stoch_k_numerator, stoch_k_denominator) # 46
        # %D - это 3-периодное SMA от %K
        # stoch_d = F.avg_pool1d(stoch_k.transpose(-1, -2), kernel_size=3, stride=1, padding=1, count_include_pad=True).transpose(-1, -2) # 47
        ...
        stoch_d = F.conv1d(padded_stoch_k, weights_3.view(1, 1, -1)).transpose(-1, -2) # 47
        ...
        # chaikin_osc = vol_sma_short - vol_sma_long # 48
        ...
        chaikin_osc = chaikin_osc_t.transpose(-1, -2) # [B, T, 1] # 48
        ...
        def compute_cci(high_t, low_t, close_t, window):
            ...
            # CCI
            cci = self._safe_division(tp - sma_tp, 0.015 * mean_dev) # 49
        ...
        ultimate_osc = compute_ultimate_oscillator(close, high, low) # 50
    ...
    def _compute_volume_indicators(self, volumes, close, window=14):
        ...
        # avg_volume_20 = F.avg_pool1d(volumes.transpose(-1, -2), kernel_size=20, stride=1, padding=10, count_include_pad=True).transpose(-1, -2) # 51
        ...
        avg_volume_20 = avg_volume_20_t.transpose(-1, -2) # [B, T, 1] # 51
        
        volume_ratio = self._safe_division(volumes, avg_volume_20) # 52
        ...
        vpt = torch.cumsum(vpt_component, dim=1) # 53
        features.append(vpt)
        ...
        obv = torch.cumsum(obv_flow, dim=1) # 54
        features.append(obv)

        # Volume Rate of Change (VROC)
        # ИСПРАВЛЕНИЕ 2: Теперь переменная window определена
        vroc = self._safe_division(volumes - torch.roll(volumes, shifts=window, dims=1), torch.roll(volumes, shifts=window, dims=1)) # 55
        vroc[:, :window] = 0.0
    ...
    def _compute_advanced_indicators(self, open_p, high, low, close, volumes):
        ...
        clv = self._safe_division((close - low) - (high - close), high - low) # 56
        adl_flow = clv * volumes
        adl = torch.cumsum(adl_flow, dim=1) # 57
        features.append(adl)
        ...
        positive_flow_sum = sum_window(positive_flow.transpose(-1, -2), 14) # 58
        negative_flow_sum = sum_window(negative_flow.transpose(-1, -2), 14) # 59
        
        money_ratio = self._safe_division(positive_flow_sum, negative_flow_sum)
        mfi = 100 - (100 / (1 + money_ratio)) # 60
        ...
        pvt = torch.cumsum(pvt_flow, dim=1) # 61
        features.append(pvt)

        # Force Index
        force_index = (close - torch.roll(close, shifts=1, dims=1)) * volumes # 62
        force_index[:, 0] = 0
        # Сглаживаем с помощью EMA
        force_index_ema = self._compute_ewma_approx(force_index, 13) # 63
        ...
        ema_13 = compute_ema(close, 13) # 64
        bull_power = high - ema_13 # 65
        bear_power = low - ema_13 # 66
    ...
    def _compute_candlestick_patterns(self, open_p, high, low, close):
        ...
        small_range = total_range < torch.roll(total_range, shifts=1, dims=1) * 0.5
        large_range = total_range > torch.roll(total_range, shifts=1, dims=1) * 1.5
        
        # Doji (дожи) - свеча с очень маленьким телом
        doji = small_body & (total_range > self.eps) # 67
        features.append(doji.float())
        
        # Marubozu (марубозу) - свеча без теней или с очень маленькими тенями
        marubozu = large_body & (upper_shadow + lower_shadow < total_range * 0.1) # 68
        features.append(marubozu.float())
        
        # Hammer (молот) - длинная нижняя тень, маленькое тело в верхней части
        hammer = (lower_shadow > body * 2) & (upper_shadow < body * 0.5) & (close > open_p) # 69
        features.append(hammer.float())
        
        # Hanging Man (повешенный) - длинная нижняя тень, маленькое тело в верхней части, но в downtrend
        hanging_man = (lower_shadow > body * 2) & (upper_shadow < body * 0.5) & (close < open_p) # 70
        features.append(hanging_man.float())
        
        # Inverted Hammer (перевернутый молот)
        inverted_hammer = (upper_shadow > body * 2) & (lower_shadow < body * 0.5) & (close > open_p) # 71
        features.append(inverted_hammer.float())
        
        # Shooting Star (падающая звезда)
        shooting_star = (upper_shadow > body * 2) & (lower_shadow < body * 0.5) & (close < open_p) # 72
        ...
        bullish_engulfing = (close > open_p) & (prev_close < prev_open) & (close > prev_open) & (open_p < prev_close) # 73
        features.append(bullish_engulfing.float())
        
        # Bearish Engulfing (медвежье поглощение)
        bearish_engulfing = (close < open_p) & (prev_close > prev_open) & (close < prev_open) & (open_p > prev_close) # 74
