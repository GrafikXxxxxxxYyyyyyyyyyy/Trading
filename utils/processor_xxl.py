...
class TradingProcessor:
    ...
    def _compute_price_changes(self, close):
        ...
        abs_close_change = torch.cat([torch.zeros_like(close[:, :1]), close[:, 1:] - close[:, :-1]], dim=1) # 15
        features.append(abs_close_change)
        
        # Относительное изменение цены закрытия (%)
        rel_close_change = self._safe_division(abs_close_change, torch.roll(close, shifts=1, dims=1)) # 16
        features.append(rel_close_change)

        # Логарифмическое изменение
        log_close_change = torch.log(torch.roll(close, shifts=-1, dims=1) / (close + self.eps) + self.eps) # 17
        ...
        close_change_2 = self._safe_division(close - torch.roll(close, shifts=2, dims=1), torch.roll(close, shifts=2, dims=1)) # 18
        close_change_2[:, :2] = 0.0
        features.append(close_change_2)

        # Процентное изменение за 5 дней
        close_change_5 = self._safe_division(close - torch.roll(close, shifts=5, dims=1), torch.roll(close, shifts=5, dims=1)) # 19
    ...
    def _compute_volatility_indicators(self, high, low, close, window=14):
        ...
        tr = torch.maximum(torch.maximum(high_low, high_close_prev), low_close_prev) # 20
        ...
        volatility = compute_std(close.transpose(-1, -2), window) # 22
        ...
        bb_mean, bb_upper, bb_lower = compute_bollinger_bands(close.transpose(-1, -2), window) # 23, 24, 25
        bb_width = bb_upper - bb_lower # 26
        bb_position = self._safe_division(close - bb_lower, bb_width) # 27
        ...
        donchian_upper, donchian_lower = compute_donchian_channels(high.transpose(-1, -2), low.transpose(-1, -2), window) # 31, 32
        donchian_middle = (donchian_upper + donchian_lower) / 2.0 # 33
    ...
    def _compute_moving_averages(self, close, volumes, open_p, high, low):
        ...
        sma_5 = compute_sma(close_t, 5) # 34
        sma_10 = compute_sma(close_t, 10) # 35
        sma_15 = compute_sma(close_t, 15) # 36
        sma_20 = compute_sma(close_t, 20) # 37
        sma_30 = compute_sma(close_t, 30) # 38
        sma_50 = compute_sma(close_t, 50) # 39
        features.extend([sma_5, sma_10, sma_15, sma_20, sma_30, sma_50])

        # EWMA (приблизительные)
        ewma_5 = compute_ewma_approx(close, 5) # 40
        ewma_10 = compute_ewma_approx(close, 10) # 41
        ewma_15 = compute_ewma_approx(close, 15) # 42
        ewma_20 = compute_ewma_approx(close, 20) # 43
        ewma_30 = compute_ewma_approx(close, 30) # 44
        ewma_50 = compute_ewma_approx(close, 50) # 45
        features.extend([ewma_5, ewma_10, ewma_15, ewma_20, ewma_30, ewma_50])
        ...
        vwap = self._safe_division(vwap_numerator, vwap_denominator).transpose(-1, -2) # 46
        features.append(vwap)

        # MACD
        ema_12 = compute_ewma_approx(close, 12)
        ema_26 = compute_ewma_approx(close, 26)
        macd_line = ema_12 - ema_26 # 47
        signal_line = compute_ewma_approx(macd_line, 9) # 48
        macd_histogram = macd_line - signal_line # 49
        ...
        hh_20, ll_20 = compute_price_channels(high_t, low_t, 20) # 50, 51
        hh_50, ll_50 = compute_price_channels(high_t, low_t, 50) # 52, 53
    ...
    def _compute_momentum_indicators(self, close, window=14):
        ...
        # Rate of Change (ROC)
        roc = self._safe_division(close - torch.roll(close, shifts=window, dims=1), torch.roll(close, shifts=window, dims=1)) # 54
        ...
        williams_r = -100 * self._safe_division(williams_r_numerator, williams_r_denominator) # 55
        features.append(williams_r)

        # Momentum
        momentum = close - torch.roll(close, shifts=window, dims=1) # 56
        momentum[:, :window] = 0.0
        features.append(momentum)

        # Price Rate of Change (PROC) - нормализованная версия Momentum
        proc = self._safe_division(momentum, torch.roll(close, shifts=window, dims=1)) # 57
        features.append(proc)

        # Rate of Change (ROC) для 5 и 20 дней
        roc_5 = self._safe_division(close - torch.roll(close, shifts=5, dims=1), torch.roll(close, shifts=5, dims=1)) # 58
        roc_5[:, :5] = 0.0
        features.append(roc_5)

        roc_20 = self._safe_division(close - torch.roll(close, shifts=20, dims=1), torch.roll(close, shifts=20, dims=1)) # 59
        roc_20[:, :20] = 0.0
    ...
    def _compute_oscillators(self, close, high, low, volumes, window=14):
        ...
        avg_gain = rma(gain, window) # 60
        avg_loss = rma(loss, window) # 61
        ...
        stoch_k = 100 * self._safe_division(stoch_k_numerator, stoch_k_denominator) # 63
        # %D - это 3-периодное SMA от %K
        # stoch_d = F.avg_pool1d(stoch_k.transpose(-1, -2), kernel_size=3, stride=1, padding=1, count_include_pad=True).transpose(-1, -2) # 64
        ...
        stoch_d = F.conv1d(padded_stoch_k, weights_3.view(1, 1, -1)).transpose(-1, -2) # 64
        ...
        # chaikin_osc = vol_sma_short - vol_sma_long # 65
        ...
        chaikin_osc = chaikin_osc_t.transpose(-1, -2) # [B, T, 1] # 65
        ...
        def compute_cci(high_t, low_t, close_t, window):
            ...
            # CCI
            cci = self._safe_division(tp - sma_tp, 0.015 * mean_dev) # 66
        ...
        ultimate_osc = compute_ultimate_oscillator(close, high, low) # 67
        ...
        trix = compute_trix(close, 15, self._compute_ewma_approx) # 68
        ...
        def compute_mass_index(high_t, low_t, ewma_func, window=25):
            ...
            mass_index = F.conv1d(padded_ratio, weights.view(1, 1, -1)).transpose(-1, -2) * window # 69
            
            return mass_index
        
        # ИСПРАВЛЕНИЕ: Передаем self._compute_ewma_approx
        mass_index = compute_mass_index(high, low, ewma_func=self._compute_ewma_approx) # 69
    ...
    def _compute_volume_indicators(self, volumes, close, window=14):
        ...
        # avg_volume_20 = F.avg_pool1d(volumes.transpose(-1, -2), kernel_size=20, stride=1, padding=10, count_include_pad=True).transpose(-1, -2) # 70
        ...
        avg_volume_20 = avg_volume_20_t.transpose(-1, -2) # [B, T, 1] # 70
        
        volume_ratio = self._safe_division(volumes, avg_volume_20) # 71
        ...
        vpt = torch.cumsum(vpt_component, dim=1) # 72
        features.append(vpt)
        ...
        obv = torch.cumsum(obv_flow, dim=1) # 73
        features.append(obv)

        # Volume Rate of Change (VROC)
        # ИСПРАВЛЕНИЕ 2: Теперь переменная window определена
        vroc = self._safe_division(volumes - torch.roll(volumes, shifts=window, dims=1), torch.roll(volumes, shifts=window, dims=1)) # 74
        vroc[:, :window] = 0.0
    ...
    def _compute_advanced_indicators(self, open_p, high, low, close, volumes):
        ...
        clv = self._safe_division((close - low) - (high - close), high - low) # 76
        adl_flow = clv * volumes
        adl = torch.cumsum(adl_flow, dim=1) # 77
        features.append(adl)
        ...
        positive_flow_sum = sum_window(positive_flow.transpose(-1, -2), 14) # 78
        negative_flow_sum = sum_window(negative_flow.transpose(-1, -2), 14) # 79
        
        money_ratio = self._safe_division(positive_flow_sum, negative_flow_sum)
        mfi = 100 - (100 / (1 + money_ratio)) # 80
        ...
        pvt = torch.cumsum(pvt_flow, dim=1) # 81
        features.append(pvt)

        # Force Index
        force_index = (close - torch.roll(close, shifts=1, dims=1)) * volumes # 82
        force_index[:, 0] = 0
        # Сглаживаем с помощью EMA
        force_index_ema = self._compute_ewma_approx(force_index, 13) # 83
        ...
        ema_13 = compute_ema(close, 13) # 84
        bull_power = high - ema_13 # 85
        bear_power = low - ema_13 # 86
        ...
        def compute_dpo(close_t, window, sma_func):
            ...
            dpo = close_t - shifted_sma # 87
            return dpo
        
        # ИСПРАВЛЕНИЕ: Передаем self._compute_sma
        dpo = compute_dpo(close, 20, self._compute_sma) # 87
        features.append(dpo)
        ...
        def compute_vortex(high_t, low_t, close_t, window=14):
            ...
            # Vortex Indicator
            vi_plus = self._safe_division(vm_plus_sum, tr_sum) # 88
            vi_minus = self._safe_division(vm_minus_sum, tr_sum) # 89
            
            return vi_plus, vi_minus
        
        vi_plus, vi_minus = compute_vortex(high, low, close) # 88, 89
    ...
    def _compute_candlestick_patterns(self, open_p, high, low, close):
        ...
        prev_open[:, 0] = open_p[:, 0]
        prev_close[:, 0] = close[:, 0]
        bullish_engulfing = (close > open_p) & (prev_close < prev_open) & (close > prev_open) & (open_p < prev_close) # 96
        ...
        bearish_engulfing = (close < open_p) & (prev_close_bear > prev_open_bear) & (close < prev_open_bear) & (open_p > prev_close_bear) # 97
        ...
        prev_high[:, 0] = high[:, 0]
        tweezer_tops = (torch.abs(high - prev_high) < self.eps) & (close < open_p) & (torch.roll(close, shifts=1, dims=1) > torch.roll(open_p, shifts=1, dims=1)) # 98
        # Исправление: заполняем начальное значение
        tweezer_tops[:, 0] = 0
        ...
        # Исправление: заполняем начальное значение
        prev_low[:, 0] = low[:, 0]
        tweezer_bottoms = (torch.abs(low - prev_low) < self.eps) & (close > open_p) & (torch.roll(close, shifts=1, dims=1) < torch.roll(open_p, shifts=1, dims=1)) # 99
        # Исправление: заполняем начальное значение
        tweezer_bottoms[:, 0] = 0
